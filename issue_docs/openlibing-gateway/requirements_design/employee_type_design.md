...
* **需求名称**: 完善uniportal账号华为用户身份判断-employeeType
* **开发责任人**:chentao
...
> 当前系统中uniportal账号不完全是内部用户，外部用户可通过uniportal注册自己账号。

> 系统中部分功能需准确判断用户是否为内部用户才能访问。

> 需要根据uniportal返回用户信息区分用户类型-employeeType。

## 3 需求验收标准

> 使用内部用户账号、和自行注册的uniportal访问只允许内部用户访问的功能，看能否识别内外部用户

## 4. 需求设计与分解

> apollo上新增配置uniportal.employee.type.list：HWE,HT,WX,KF,GW,WY,FW，内部员工（HWE,HT），合作账号（WX,KF,GW,WY,FW）

> 数据库表user_info_uniportal新增Boolean型字段internal_user，用于判断用户是否为内部用户（1：是，0：否）。

> 需准确判断用户是否为内部用户，通过数据库表user_info_uniportal中的internal_user字段判断。


### 4.1 核心逻辑方案

>1、uniportal返回用户信息employeeType,与配置uniportal.employee.type.list进行匹配，判断用户是否为内部用户

>2、更新数据库表user_info_uniportal的internal_user字段，判断用户是否为内部用户

>3、后续业务校验用户是否为内部用户，通过数据库表user_info_uniportal中的internal_user字段判断。
...
|-----------|----------------------------------------------------------------|---------------------|-----------|
| **task1** | apollo上新增配置uniportal.employee.type.list                        | 配置文件                | 0.5       |
| **task2** | 数据库表user_info_uniportal新增Boolean型字段internal_user，用于判断用户是否为内部用户 | 数据库表                | 0.5       |
| **task3** | 修改原判断内部用户逻辑                                                    | 配置文件/代码             | 1         |
...
| **工作量**   | 预计总工作量 **** xx 人天？        | 2人天       | 
| **价值评估**  | 实现后能减少多少手动操作或提升多少系统稳定性？         | 提升账号识别准确性 |
