...
> openlibing业务运行时关键接口/核心业务逻辑无告警通知，需要设计并开发通用告警模块，支持各个服务接口对接

## 3 需求验收标准

> openlibing的关键业务接口加上@Logapi的注解，并配置注解参数isSendAlarm为true后，当前服务接口在发生异常时即可发送告警信息

## 4. 需求设计与分解

> 修改openlibing-common的的日志切面方法，判断发生异常时调用消息通知接口进行告警
> 对接华为云SMN服务的消息通知服务接口

### 4.1 核心逻辑方案

> 1、修改openlibing-common的日志切面告警方法，若isSendAlarm为true，则调用消息通知接口进行告警
> 2、新建类及方法，构造参数消息体，并调用华为云SMN服务的消息通知服务接口
...
|-----------|------------------------------|---------------------|-----------|
| **task1** | openlibing-common的日志切面告警方法改造 | 代码                  | 5         |
| **task2** | 对接华为云SMN服务的消息通知服务接口          | 代码                  | 5         |
| **task3** | 服务发布与验证                      | 配置文件                | 3         |
