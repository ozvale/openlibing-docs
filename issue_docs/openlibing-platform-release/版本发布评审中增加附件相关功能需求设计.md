...
| 文件类型异常 |基于文件扩展名进行校验|500|附件类型不支持，支持pdf,doc,docx,xls,xlsx,zip,rar,7z,tar,tar,gz类型|
...
2. **文件类型校验**: 基于文件扩展名校验
3. **权限校验**： 基于评审单查询创建者以及相关人员。非创建者，无法进行上传/删除，非评审单相关人员无法进行下载
4. **数量限制校验**: 基于数据库查询当前评审单的附件数量
5. **OBS桶存储**：服务端容器不落盘，直接存储到OBS桶中
6. **错误处理**: 基于异常情况提供清晰的错误提示信息，并记录日志
...
public class ReleaseAttachmentServiceImpl implements ReleaseAttachmentService {
    ...
    public DataResult<String> uploadAttachment(String userId, Integer releaseId, MultipartFile attachmentFile) {
        ...
        // 4. todo评审单状态是否在新建状态
        // 5. todo校验上传数量
        // 6. todo上传到OBS桶，同名文件上传直接覆盖
        // 7. todo记录数据库，同名文件进行更新时间，重新上传已被删除的同名文件，不更新重新增加记录
...
| attachment.release.allowedTypes | 允许的附件类型      | pdf,doc,docx,xls,xlsx,zip,rar,7z,tar,tar.gz |
