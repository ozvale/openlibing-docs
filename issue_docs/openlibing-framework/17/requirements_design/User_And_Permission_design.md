...
> 步骤2：后端接口：（1）菜单功能配置：接口403异常，触发提示模块（menu_url_info表添加trigger_permission字段。历史数据：默认值空-不处理403情况；0-不处理403情况；1-触发权限申请弹框；2-触发无权限提示页面）；（2）提供一个接口，入参为当前403异常接口URL；（3）根据菜单配置进行校验，返回接口403处理情况；
