...
> 调用codebuddy和lingma的公开接口，获取企业用户的用户信息，企业用户的使用信息。
...
> 在openling-sync中实现数据拉取的逻辑，在openling-metric中实现数据查询接口

### 4.1 核心逻辑方案

> 数据拉取采用策略模式，不同的数据，使用不同的策略进行数据拉取，将基础数据存入doris库
> 每天凌晨拉取前一天的使用数据
> 拉取数据结束后，进行数据清洗，将需要的使用数据，都汇总到clean表中
...
|-------------------|-------------------------|--------------------|-------------|
| **codebuddy数据拉取** | **拉取codebudy用户数据和使用数据** | 配置文件/代码            | 2           |
| **lingma数据拉取**    | **拉取lingma用户数据和使用数据**   | 配置文件/代码  | 2 |
| **数据清洗**          | **拉取的数据进行清洗**           | 核心逻辑代码             |  1 |
| **数据接口**          | **通过机机接口暴露接口给黄区**        | 核心逻辑代码             |  2 |
