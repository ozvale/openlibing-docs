...
| `PipelineService` / `PipelineServiceImpl`                                                                                                                          | 流水线业务逻辑        | 新增名单查询/保存/标记查询方法；`runPipeline`、`retryPipeline` 叠加名单校验；新增 `retryPipelineByEvent`（事件链路专用，跳过名单校验）；新增统一评估方法（门1 + 门2 收口，flags 投影调用）；新增成员 VO 转换与项目成员查询辅助方法 |
| `GitVisibilityService`                                                                                                                                             | 仓库公开性共享服务    | 新增：从 `AuthInterceptor` 迁移 `isGitUrlPublic` / `checkGitUrlPublicFromHwCloud`（含 Redis 缓存与异步刷新逻辑），`AuthInterceptor` 改调本服务，消除双实现漂移隐患 |
