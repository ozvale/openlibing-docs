...
v5 方案：`user_role_info` 加 `group_id` 列（存量置 0 = 默认权限组），资源绑定自定义权限组后鉴权按组过滤；能力落在 framework 层，后续 CodeCheck、CodeRepo 可复用。
