...
### 前端（openlibing-web，已交付；2026-08-24 两次方案更新：弹窗改为按钮旁常驻提示；文案与购买链接改版）
...
2. 若包含，在保存按钮旁显示 warning 提示框：`包含“代码检查安全增强”类规则，请确保已购买增强规则集包。`，其中“增强规则集包”为华为云购买链接（`https://console.huaweicloud.com/devcloud/?region=<region>#/subscribe/apply?packageType=feature&version=`，新窗口打开；`region` 取自社区对接的华为云项目，来源接口 `/openlibing-framework/project/get-project-detail-info` 返回的 `data.hwProjectEntity.region`，未获取到时默认 `cn-southwest-2`）
...
- [ ] 勾选安全增强类规则后，保存按钮旁实时出现 warning 提示框：`包含“代码检查安全增强”类规则，请确保已购买增强规则集包。`，“增强规则集包”可点击跳转华为云订阅页（region 与社区对接的华为云项目一致，新窗口打开）
- [ ] 取消勾选后提示框实时消失
