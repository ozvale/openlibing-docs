...
- **Issue 关联方式**：暂不关联
- **创建日期**：2026-08-27
- **最近同步**：2026-08-31（`feature/nopermission` `cb8f8842` 已合入 `release_20260831`；后续 `fix-permission-bubble-and-offline-notice` 分支 `a4f622ee` 回退了工具市场接入，见 AC5）
...
- **后续回退**（`fix-permission-bubble-and-offline-notice` 分支提交 `a4f622ee`）：移除 ToolMarket 三个按钮的 `NoPermissionPopover` 包裹，恢复为纯 `:disabled` 置灰控制（无 hover 气泡）。该 AC 不再有效。
