...
新增 Liquibase 变更集 `20260813_default_can_cross_project_tool_version`，将 `tool_version` 表中 `can_cross_project` 为 NULL 或空字符串的行更新为 `'1'`（需审核），确保存量数据默认安全。

```sql
UPDATE tool_version
SET can_cross_project = '1'
WHERE can_cross_project IS NULL OR can_cross_project = ''
```
