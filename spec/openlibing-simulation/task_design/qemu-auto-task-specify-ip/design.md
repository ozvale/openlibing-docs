...
1. 该接口是异步的（`saveAutoQemuTask` 提交线程池后立即 `return 200`），因此"不满足即接口返回失败"
   只能靠同步段的预检实现。
2. **同步预检只校验"指定机器存在"一件事**（已确认：同步不判 CPU、内存余量，也不做规格解析与部署单元数校验）。
...
| **同步预检范围**        | 只校验：指定 IP 的机器存在（`t_server_basic_info.is_deleted='0'`）；**不**做规格解析、**不**做部署单元数校验                                                                                         | **已确认：同步不判 CPU / 内存余量**；且当前业务只有单机场景，部署单元数恒为 1，无需校验                                                                                        |
...
| **失败契约**            | 同步失败 `code = 2003`，错误信息入 `message`，`data` 为空（仅 1 类原因，见第 4 节）                                                                                                                  | 已确认需求口径；`2003` 为本仓新增业务码。**注意：资源不足不体现在同步响应码中**                                                                                                |
...
| **`creator` 过滤**      | 指定 IP 分支不带 `creator` 过滤；自动选机 SQL 原有的 `creator='triton'` 硬编码**一并移除**                                                                                                           | 已确认；指定机器由用户显式给出，与 `creator` 语义无关；自动选机侧该硬编码已无业务含义                                                                                          |
| **指定 IP 分支的口径**  | 只按 `info.ip = #{serverIp}` 取机 + 同口径资源筛选，**跳过** `subScene`、`architecture` 过滤                                                                                                         | 用户显式指定机器 = 用户对机型负责                                                                                                                                              |
| **同步预检的入参**      | 只传 `serverIp`，**不需要** 部署规格                                                                                                                                                                 | 预检已不判规格与部署单元数，`engineId` / `yamlManageId` / `nodeNumber` 不再参与                                                                                                |
...
| **多机场景**            | 不做同步拦截：指定 IP 时仍按"每个部署单元各自取机"走异步链路，失败由既有"环境不足 → 任务置 FAILED"暴露                                                                                               | 当前业务只有单机场景（已确认），无需为不存在的场景增加同步分支                                                                                                                 |
...
| `service/NodeManageService.java`                        | 修改 | 新增公开方法 `checkSpecifiedServer(serverIp)`                                                                                                                      |
| `service/impl/NodeManageServiceImpl.java`               | 修改 | 实现上述公开方法；`createQemuEnv` 反查并回填 IP；`getServerNodes` 增加"指定 IP 优先"分支                                                                           |
| `mapper/QemuTaskMapper.xml`                             | 修改 | `insertOrUpdateConfig` 的 insert 列 / `values` / `on DUPLICATE KEY UPDATE` 三处补 `server_ip`                                                                      |
| `mapper/NodeManageMapper.java` + `NodeManageMapper.xml` | 修改 | 新增 `getSpecifiedServerIpByTaskId`、`countServerByIp`、`getServerFreeResourceByIp` 三个查询；`getDynamicComputeFreeServerIp` 去掉 `creatBy` 形参与 `creator` 过滤 |
| `test/.../QemuTaskControllerTest.java`                  | 修改 | 补携带 `config.serverIp` 的用例                                                                                                                                    |
| 同步预检单测类                                          | 新增 | 覆盖"机器存在 / 不存在"两类 + 余量不足的反向用例                                                                                                                   |
...
QemuTaskServiceImpl.saveAutoQemuTask                       #248
      ...
      │      │
      │      └─ serverIp 非空
      │            └─ nodeManageService.checkSpecifiedServer(serverIp)
                                ...
                                └─ allocateNodeWithLock（全局锁 node_allocation_lock，保持原样）
                                      └─ allocateNode → checkEnvEnough → exclusiveIsEnoughEnv
                                            └─ getServerNodes                        #312
                                                  ...
                                                  └─ serverIp 为空  ──▶ 原逻辑 getDynamicComputeFreeServerIp(simulationDeploy, nodeIds)
                                                        │
                                                        ▼
...
// 【新增】指定机器同步预检（方案 A：轻量预检、纯读、不加锁；只校验机器存在）
 ...
 */
ResponseEntity checkSpecifiedServer(String serverIp);
```

`NodeManageServiceImpl` 实现：

```java
@Override
public ResponseEntity checkSpecifiedServer(String serverIp) {
...
> **已取消的两项校验（2026-09-18 第二轮修订）**：初版设计在同步段还做"部署规格可解析"与"部署单元数 == 1"两项校验，
> 并为此新增按 `engineId + yamlManageId` 解析规格的查询 `getQemuDeploySpec` 与私有辅助 `getDeployResultListByConfig`。
> 经确认当前业务**只有单机场景**，且规格能否解析最终会由异步选机链路自行暴露失败，
> 因此同步段不再做这两项：`getQemuDeploySpec` 与 `getDeployResultListByConfig` **未落地**（已从实现中删除），
> 预检入参收敛为只有 `serverIp`（不再需要 `config`）。
...
同步预检只有 **1 类**失败消息（英文，与 `checkParam` 既有风格一致）：
...
`未找到符合cpu内存的机器`）。这与 Issue #31 扩展场景 2 的原文不一致，**已同步修订 Issue 文案**（见第 7 节）。
...
- 相比自动选机 SQL，**刻意去掉** `ext.extend_value like '%subScene%'`、`architecture` 两个过滤（决策见架构决策表）；
  `creator` 过滤两处都不带——自动选机 SQL 原有的 `creator='triton'` 硬编码已在本轮一并移除（见第 7 节）。

### 7. 需求偏差：Issue #31 已修订（2026-09-18 完成，两轮）

初版 Issue 文案与"只在异步判"口径冲突，已通过 `gitcode issue edit 31` 修订并复核，当前文案与本次实现一致：

| Issue #31 初稿                                                                                                                              | 修订后（现行文案）                                                                 |
| ------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| 主成功场景 2：系统在同步阶段执行预检：校验指定 IP 的机器存在，**且剩余 CPU/内存满足部署需求**                                               | 同步阶段只校验"指定 IP 的机器存在"；资源余量在异步选机阶段判定                     |
...
| 风险                                               | 影响                                                                | 缓解措施                                                                                                                                               |
| -------------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **资源不足不再同步返回失败**                       | 上游收到 200 后需靠任务状态判断成败；若上游只认同步码，会误判为成功 | 已确认取舍；Issue #31 已按此口径修订（第 7 节），仍需通知上游；任务置 FAILED 时会写 `error_message` 与任务日志，可供上游轮询                           |
...
| **自动选机去掉 `creator='triton'` 后候选范围扩大** | 自动部署任务可选中任意创建人的机器，与改造前行为不同                | 已确认（该硬编码已无业务含义）；余量与机型过滤保持不变，不会超分；上线后建议观察自动选机命中情况                                                       |
...
- 新增失败码 `2003`（仅 1 类原因）：上游若对 `code` 做白名单判断，需放行 2003（当前失败码为 400/500，本次新增一类）
- **行为变更提示**：资源不足不再体现在同步响应码中，上游需按任务状态判断成败（若依赖"同步失败即重试"，需调整）
- **行为变更提示**：自动选机不再限定 `creator='triton'`，候选机器范围扩大（对所有 QEMU 自动部署任务生效）
...
必须核对 `target/surefire-reports/` 是否有失败项，不能只看构建结果。另需通过 pre-commit
（trailing-whitespace / end-of-file-fixer / check-yaml,json,xml / check-merge-conflict /
check-added-large-files / detect-private-key / gitleaks / ruff-format / ruff-check，**无 google-java-format**）。

## 监控与日志

**关键日志点**：

- 同步预检不通过：`log.info`，含 `serverIp`（仅有"机器不存在"一类原因）
...
1. **Issue #31 文案已修订（两轮）**：见第 7 节；`the specified server has insufficient cpu or memory` 契约已移除。
2. **`getEngineTypeByTaskId` 结果未被使用**：`NodeManageMapper#getEngineTypeByTaskId`（`NodeManageMapper.java` #46）
   被 `createQemuEnv` #113 调用后传给 `getDeployResultList`，但后者实现内并未使用该形参（实际读 `qemuDynamic.getType()`）。
   本次不动，避免扩大改动面。
3. **`getServerNodes` 的 `creatBy` 形参**：原本就未被使用（自动选机 SQL 的 `creator` 过滤已移除），
   形参仍在但已无实际作用，可后续单独清理（删除会牵连反射取方法的单测签名）。
