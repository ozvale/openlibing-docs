...
- 业务 Issue：[#27](https://gitcode.com/openlibing/openlibing-simulation/issues/27)「【蓝区】湛卢仿真运营管理接口与数据上报能力建设」US1（仿真机器/引擎/qcow 标准化 CRUD 接口）。
  说明：Issue [#26](https://gitcode.com/openlibing/openlibing-simulation/issues/26)（提供仿真机器/引擎/qcow 增删改接口）范围与 US1 重叠，交付统一并入 #27 的业务 PR。
- 现状：5 张资源表的实体已存在，但模块内**没有任何增/改/删 SQL 与管理端点**（Controller 仅任务与健康检查），全部写路径缺失。
- 目标：为以下 5 张表提供 RESTful 增删改接口：
  `t_server_basic_info`、`t_server_extend_info`、`t_engine_basic_info`、`t_qcow_info`、`t_resource_attachment`
  （范围变更：`t_server_using` 不开放管理增删改，沿用既有分配/释放流程写入）
...
- [ ] 业务 PR 关联 issue #27，打 ai-assisted 标签
