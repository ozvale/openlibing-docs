...
watch(app.projectInfo.projectId)
  └─ restoreFilterCache()
       ...
       └─ getPipelineGroupTree() → 重新加载分组树与列表
             └─ 3. 树加载完成后校验缓存分组有效性
                   ├─ 非分组模式（byGroup = false）→ 无需校验
                   ├─ 按 path_id 在树中找到分组 → 用树中最新节点覆盖
                   │  currentPipelineGroup（刷新 children，避免缓存陈旧）
                   └─ 未找到（分组已被删除/重命名）
                         → byGroup = false，currentPipelineGroup 回退全部分组，
                           并以默认条件重新查询列表
```

**重置优先设计的原因**：若直接读缓存，目标项目无缓存时上一项目的分组/筛选/分页状态残留，列表被错误过滤；先无条件重置保证任意情况下都回到干净的默认态，再由缓存覆盖。

**分组失效校验的原因**：分组按项目维护，会话期间分组可能被删除或重命名。若恢复了一个已不存在的分组，页面会停留在分组模式下却查不到对应列表内容（空列表或查询异常）。分组树加载完成后按 `path_id` 校验，失效则回退全部分组并以默认条件重新查询，保证恢复流程对分组失效场景同样收敛到干净状态。
