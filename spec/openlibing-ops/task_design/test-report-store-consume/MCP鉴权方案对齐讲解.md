...
| **MCP Server** | 提供方：真正跑着、对外提供工具的服务 | 我们改的 ops 服务（最终为 `/mcp/sse`，SSE 传输；本文档前面为简化按 `/mcp` 泛指） |
| **MCP 市场** | 中介：只登记信息 + 发配置，不提供能力 | 应用商店（只上架，不跑 App） |

**关键**：本需求是给 ops 服务加 MCP 能力（最终为 SSE 传输 `/mcp/sse`），把"查测试报告数据"暴露成 MCP 工具，让 AI 客户端能调用。
