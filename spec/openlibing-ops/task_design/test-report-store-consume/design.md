...
> **审计与鉴权的依赖关系（2026-08-24 明确）**：日志表**不依赖鉴权即可落地**，但"行为记录"与"安全审计"两层能力不同——
> - 无鉴权时：工具名/参数/耗时/成败照记（**运营观测**：调用量、工具热度、性能、故障定位可用），但 `caller` 只能记 IP/User-Agent，**无法落到账号/客户**，**安全审计（谁在调/越权追责）做不了**；且匿名可灌脏数据污染日志。
> - 有鉴权时：`caller` 可由 JWT/token 解析出真实身份（OAuth 用户级或应用级），日志升级为**安全审计**（越权判定、责任追溯）。
> - **结论**：8.2.2 日志表该建照建（横切关注点，不依赖鉴权），但在无鉴权阶段仅当"运营观测"，正式鉴权（8.2.3 ①）落地后才具备安全审计价值；`caller` 字段为鉴权后的身份预留。
