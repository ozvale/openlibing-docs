# Design: pytest-orch GitCode Actions Plugin Development
...
| 参数名                 | 类型   | 必选 | 校验规则                                                               | 用途                 |
| ---------------------- | ------ | ---- | ---------------------------------------------------------------------- | -------------------- |
| git-username           | string | 是   | 1-128 字符,字母数字下划线中划线点@                                     | GitCode 用户名       |
| git-password           | string | 是   | 1-512 字符,非空                                                        | GitCode 密码或 Token |
| workspace              | string | 否   | 默认 "workspace",1-64 字符,字母数字下划线中划线                        | 工作目录名           |
| testcase-config-file   | string | 否   | 路径安全校验(相对或绝对路径,允许/home、/tmp、/workspace)               | 测试配置文件路径     |
| log-conf               | string | 否   | JSON 字符串或文件路径                                                  | 日志配置             |
| scheduler-secret       | JSON   | 是   | 最大 10KB,最大深度 10,字段白名单(apig_code,apig_key,apig_secret等)     | 调度器密钥           |
...
| obs-base-url           | string | 是   | HTTP/HTTPS URL,最大 2048 字符                                          | OBS 基础 URL         |
| executor-branch        | string | 否   | 默认 "master",分支名格式(字母数字下划线中划线斜杠点)                   | Executor 仓库分支    |
| executor-repo          | string | 否   | 默认 openlibing/openlibing-pytest-executor,owner/repo 格式             | Executor 仓库地址    |
| testcase-repo          | string | 是   | owner/repo 格式                                                        | 测试用例仓库         |
| case-branch            | string | 否   | 默认 "main",分支名格式                                                 | 测试用例分支         |
| max-workers            | string | 否   | 默认 "2",正整数                                                        | 最大并发数           |
| pytest-testkit-url     | string | 否   | 默认 GitCode Release URL,HTTPS协议,gitcode.com域名,.whl文件            | testkit 下载地址     |
| testcase-collector-url | string | 否   | 默认 GitCode Release URL,HTTPS协议,gitcode.com域名,.whl文件            | collector 下载地址   |
| image-label            | string | 否   | 标识符格式(字母数字下划线中划线点)                                     | 镜像标签             |
| archive-log-dir        | string | 否   | 路径安全校验                                                           | 归档日志目录         |
| env-model              | string | 否   | 标识符格式                                                             | 环境模型             |
| env-device-resource    | JSON   | 否   | 最大 10KB,最大深度 10,字段名格式校验,值类型校验(字符串/数字/布尔/null) | 设备资源配置         |
| env-deploy-model       | string | 否   | 默认 "Dislocated",枚举值(Co-located,Dislocated)                        | 部署模式             |
| platform               | string | 否   | 默认 "codearts",标识符格式                                             | 平台前缀             |
...
开始
  ...
  │   └─ 克隆到 pytest-infra 目录
  │
  ├─ Step 5: 下载 pytest-testkit wheel
  │   ├─ 检查 URL 是否提供
  │   └─ wget 下载 wheel 文件
  │
  ├─ Step 6: 下载 testcase-collector wheel
  │   ├─ 检查 URL 是否提供
  │   └─ wget 下载 wheel 文件
  │
  ├─ Step 7: 克隆测试用例仓库
  │   ├─ 检查目录是否存在
  │   ├─ 安全删除旧目录
  │   └─ git clone 到 pytest-infra 目录
  │
  ├─ Step 8: 验证 main.py 存在
  │   └─ 检查 executor-dir/pytest-executor/main.py
  │
  ├─ Step 9: 创建虚拟环境
  │   ├─ 检测包管理器(apt-get/dnf)
  │   ├─ 安装 python3-venv(如需要)
...
- `.gitcode/actions/pytest-orch/index.js` (1291 行)
- `.gitcode/actions/pytest-orch/package.json` (26 行)
- `.gitcode/actions/pytest-orch/package-lock.json` (145 行)
- `.gitcode/actions/pytest-orch/dist/index.js` (打包后)
- `.gitcode/actions/pytest-orch/dist/licenses.txt` (依赖许可证)
