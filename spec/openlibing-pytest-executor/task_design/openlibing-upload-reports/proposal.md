...
- [x] 能够通过 GitCode Actions 配置文件调用该插件
- [x] 支持 Pipeline 模式上传（需要 pipeline-id、pipeline-run-id、job-id）
- [x] 支持 Label 模式上传（需要 label，可选 archive-path）
- [x] 能够正确验证文件路径，拒绝路径遍历攻击
- [x] 能够正确解析 JSON 参数，只允许白名单字段
- [x] 能够正确验证 label 和 archive-path 参数
- [x] 上传成功后返回 success=true 和正确的 HTTP 状态码
- [x] 上传失败时返回 success=false 和错误信息
- [x] 支持 space-separated 的多文件上传
- [x] 正确处理文件不存在的情况（跳过并警告）
