...
| 8 | **开关配置 OR 聚合** | 后端 | 下游使用开关类配置时，先校验该 repo_url 是否存在重复录入，存在的话对多行开关取 **OR**（MAX：有一开启则开启，全关闭才关闭）；**不修改其他项目行**，仅在读取时聚合；新增 `POST /project-repo/internal/aggregate-switch` 内部接口供下游仓调用 |
...
| 12 | **新增 `project_repo_global_config` 全局配置表**，从 `project_gitcode_role_mapping` 迁移数据（存量 role_mapping 迁入 `config_json[platform].roleMapping`，迁移完成后旧表废弃） | 数据模型 | 公共账号 / sig-info 路径 / 角色映射统一存新表 `config_json`（sigInfoLocations 顶层数组 + 角色映射按平台分键）；**建表与数据迁移由 Liquibase 同一 changeset 定义，随实例启动原子执行（无「上线后-迁移前」读空空窗）**；**framework 删产品/项目级联清理改指新表（例外仓，建议与 coderepo 同迭代上线，见 requirement-design.md §4.2）** |
...
| 序号 | 页面 | 路径 | 改动类型 |
|------|------|------|---------|
| 1 | 代码仓管理列表页 | [Repos/index.vue](file:///d:/Develop/Java/openlibing-web/apps/web-openlibing/src/views/Repos/index.vue) | 录入/编辑交互改造 |
...
- **代码仓录入配置**（独立区域，不限定在页签内）：
  - 维护**多个** sig-info.yaml 路径（配置与提交不区分平台，后端根据 URL 域名统一区分）；路径指向目录（如 `https://gitcode.com/openlibing/community-private/blob/master/openLiBing-private/sigs/openLiBing-private`），在该目录下找 sig-info.yaml 文件；**GET 回显按平台分组**（gitcode/gitee/github 外层 key，前端易区分；提交时将各组 url 拍平为 URL 数组）
...
- **实现**：coderepo 新增 `POST /project-repo/internal/aggregate-switch` 内部接口供下游仓调用（按 repo_url 聚合返回开关值）；**本期下游 7 仓零改动**——聚合逻辑由 coderepo 侧在提供数据时统一处理。
...
> 权限矩阵基于 [接口权限配置.xlsx](file:///d:/Develop/Java/接口权限配置.xlsx) 中 `/openlibing-coderepo/project-repo/*` 接口的现有角色配置，并新增 SIG 一键同步等接口的权限。
>
...
| 4 | 全局配置可保存/查询：GET 回显 sig-info.yaml 路径**按平台分组**（gitcode/gitee/github 外层 key）+ 三平台页签配置项目公共账号（GitCode 另含角色映射）；POST 提交 URL 数组 | UI 操作 + DB 验证 `project_repo_global_config.config_json` 记录 |
...
| 9 | **开关配置 OR 聚合**：同 repo_url 多行中任一行开关开启 → `aggregate-switch` 接口返回该开关为开启；全部关闭才返回关闭；**存储值不变（不修改其他项目行）** | 接口 + DB 验证 |
