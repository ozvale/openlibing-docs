...
1. **收敛配置漂移（模型不拆分，主仓仅用于录入时一键同步）**：经与 SCA 确认，其扫描按 `(project_id, repo_url)` 反查 `repo_info`、查不到即报错。因此 `repo_info` **保持「一项目一行、多行并存」现状模型**，每行 `project_id`=该行所属项目，7 个下游仓零改动。**主仓为内部标记（`is_main_repo`），仅用于录入代码仓时一键同步配置项**——同 repo_url 新行默认复制主仓配置，避免重复手填；不参与扫描归属、权限判定；各项目行独立维护，编辑不做全组覆盖（见 requirement-design.md §1.2）。
2. **双录入通道**：
   - 手动录入：保留现有方式，输入 `repo_url` 后自动检测。**首次录入默认即主仓，不提供「是否为主仓」配置项**；若该仓库已在其他项目录入，则自动按**主仓配置**同步进表单（可修改），支持「选择性删除其他项目行」；仅在**未删除之前项目行**时提示「是否需要将当前仓设为主仓」，并明确告知主仓概念仅用于录入代码仓时一键同步配置项。
...
| 3 | `/project-repo/add-repo` 增加「命中多行」分支 | 接口 | blur 检测命中其他项目已录入 → 复制主仓配置进表单（可修改）；提交可选 `setMainRepo`（仅录入时设为本项目主仓，只迁移标记）与 `deleteProjectIds`（选择性删除其他项目行）；首次录入不提供主仓配置项（默认即主仓） |
| 4 | `/project-repo/update-repo` 直接编辑（移除 `setMainRepo`） | 接口 | 不做 SIG 来源拦截；**仅更新本行配置，不做全组同步**（编辑不涉及主仓） |
| 5 | `/project-repo/delete-repo` / `batch-delete-repo` 增加 `projectId`（及可选 `newMainRepoId` / `newMainRepoMap`）入参 | 接口 | 按行删除；删除主仓行时——组内其他行=1 → 自动迁移主仓后删除（不提醒）；组内其他行≥2 → 前端选择新主仓后同事务迁移+删除 |
| 6 | 新增 `/project-repo/check-repo-url` | 接口 | 手动录入冲突预查（返回主仓项目 + 已关联项目列表） |
| 7 | 新增 `/project-repo/set-main-repo` | 接口 | 迁移主仓标记（**仅改 `is_main_repo`，不覆盖任何行配置**）；用于录入设为主仓 / 删除主仓行前选择新主仓 |
| 8 | 新增 `/project-repo/get-repo-association` | 接口 | 删除/录入前查关联项目列表（判定主仓行删除迁移规则 + 关联项目数） |
| 9 | 新增 SIG 系列接口（config/repos/import） | 接口 | SIG 位置配置（每平台一个链接）+ 一键录入 |
| 10 | 录入对话框「录入方式」切换（手动/SIG） | 前端 | 见页面原型场景二、四 |
| 11 | 手动录入：blur 自动检测 + 命中多行未删除时提示设为主仓（主仓仅用于录入时同步）+ 选择性删除 | 前端 | 见页面原型场景二；无「一键同步」按钮、无「修改同步全组」提示 |
| 12 | 编辑对话框：仅更新本行配置、不涉及主仓 + 不做 SIG 来源拦截 | 前端 | 见页面原型场景三 |
...
| 3 | 编辑代码仓对话框 | index.vue 内联 `el-dialog` | 改造：仅更新本行配置，不涉及主仓；不做 SIG 来源拦截 |
...
| 6 | 删除代码仓对话框 | index.vue 内联 `el-dialog` | 小幅改造：删除主仓行时组内其他行≥2 弹「请选择一个仓为主仓」选择框 |
...
- `repoUrl` blur 后自动调检测接口：命中该仓库已在其他项目录入 → 蓝色提示条（列出已关联项目与主仓，表单已按主仓配置自动同步，可修改）+「删除其他项目中的该仓库」多选（可多选，非一次删除所有）；**未勾选删除之前项目行时**提示「是否需要将当前仓设为主仓」，并说明**主仓仅用于录入代码仓时一键同步配置项**
- **首次录入（全局首录）不显示任何主仓相关项**（默认即主仓）；无「一键同步」按钮、无「修改同步全组」提示（编辑不涉及全组同步）

**场景三：编辑代码仓 - 仅更新本行**
- 打开编辑时不涉及主仓设置、不做 SIG 来源拦截：仅更新本行配置，各项目独立维护本行（主仓仅用于录入时一键同步）
- 不做多项目影响二次确认（删除时若涉及主仓行迁移见删除对话框）
...
- 输入 `repoUrl` blur 调 `check-repo-url`（详见 requirement-design.md §2.1）：
  - **全局首次录入**：新建本行（`is_main_repo=1`，**首次录入默认即主仓，不提供主仓配置项**），同步平台元数据 + 配置 webhook。
  - **已在其他项目录入**：自动按**主仓配置**同步进表单（可修改）。提交时：
    - 当前项目已有本行 → 更新本行配置（**仅本行，不做全组同步**）。
    - 当前项目无本行 → 新建本行（`is_main_repo=0`，配置=主仓配置副本）。
    - **未勾选删除之前项目行时**提示「是否需要将当前仓设为主仓」（`setMainRepo`，仅录入时）→ 仅迁移 `is_main_repo` 标记到本行，**不覆盖任何行配置**。
    - 可选「删除其他项目中的该仓库」（`deleteProjectIds`）→ 删除对应项目行（含主仓行时按 §5.4 自动/选择迁移主仓）。
- **无「一键同步」按钮、无「修改同步全组」提示、不做来源拒绝**（任何仓库均可录入关联到当前项目）。
...
- 打开编辑时**不涉及主仓设置**：仅更新本行配置，各项目独立维护本行（主仓仅用于录入时一键同步，编辑不做全组覆盖）。
- 不做多项目影响二次确认、不做 SIG 来源拦截（详见 requirement-design.md §2.3）。

### 5.4 删除

- **按行删除**（每行独立，其他项目行不受影响）；入参增加 `projectId`（越权校验）。
- **主仓行删除自动/选择迁移**：组内其他行=1 → 后端**自动迁移主仓**到该行（不提醒用户）后删除；组内其他行≥2 → 前端弹「请选择一个仓为主仓」选择框，删除请求携带 `newMainRepoId`，后端同事务迁移主仓后删除（迁移**仅改 `is_main_repo` 标记，不影响任何行配置**）；组内仅剩本行 → 直接删除（组消失）。
...
- **Phase 1（本期上线时执行，幂等可重跑）**：清洗同项目重复行 → 标记主仓（最早 create_at 行）→ 加 `(repo_url, project_id)` 唯一索引；**不归并、不删行、不建关联表、不删 `project_id`** → 7 个下游仓零影响。
...
| 代码仓管理 | 仓库管理 | 录入仓库 | 手动录入仓库（首次即主仓；命中多行可设为主仓 / 选择性删除） | 平台运维人员 | ✅ | ✅ |
| 代码仓管理 | 仓库管理 | 录入仓库 | 同上 | 产业管理者 | ✅ | ✅ |
| 代码仓管理 | 仓库管理 | 录入仓库 | 同上 | 项目审批人员 | ✅ | ✅ |
| 代码仓管理 | 仓库管理 | 录入仓库 | 同上 | 流水线工程师 | ✅ | ✅ |
| 代码仓管理 | 仓库管理 | 录入仓库 | 同上 | 项目管理员 | ✅ | ✅ |
| 代码仓管理 | 仓库管理 | 修改仓库信息 | 编辑仓库配置（仅更新本行，不涉及主仓；SIG 来源同样可编辑） | 产业管理者 | ✅ | ✅ |
...
| 序号 | 验收项 | 验收方式 |
|------|--------|----------|
| 1 | **Phase 1** 迁移完成：同项目重复行已清洗；每 `repo_url` 组合法主仓数=1；`(repo_url, project_id)` 唯一索引生效；7 个下游仓按 `(project_id, repo_url)` 反查全部命中 | 迁移脚本校验输出 + DB 查询 + SCA 联调用例 |
| 2 | 手动录入输入已存在 repo_url 后 blur 自动检测，自动按主仓配置同步到表单（可修改）；**首次录入默认即主仓且不显示任何主仓配置项**；无「一键同步」按钮 | UI 操作验证 |
| 2b | 手动录入命中多行且**未勾选删除之前项目行**时提示「是否设为主仓」（并说明主仓仅用于录入时一键同步配置项）；支持选择性删除其他项目行；无「修改同步全组」提示 | UI + 接口验证（setMainRepo / deleteProjectIds） |
| 3 | 手动录入提交后当前项目可见该仓库；同 repo_url 新行默认复制主仓配置（可修改）；**编辑仅更新本行、不改其他行配置** | UI + DB 验证 |
...
| 8 | 编辑多项目已录入仓库：仅更新本行配置（不涉及主仓、不做全组覆盖）；不做多项目影响二次确认 | UI + DB 验证 |
| 9 | 删除主仓行：组内其他行=1 → 自动迁移主仓后删除（不提醒用户）；组内其他行≥2 → 选择新主仓（newMainRepoId）后同事务迁移+删除；组内仅剩本行 → 直接删除 | UI + 接口验证 |
...
| 存量 53 个共享仓多行未归并 | 配置漂移无法彻底收敛 | 录入新行默认复制主仓配置（一键同步）避免重复手填；存量配置差异需人工按需调整；Phase 1 不归并（对 7 仓零影响）；归并推迟到 Phase 2 7 仓逐个可控后统一执行 |
| 主仓标记迁移并发覆盖 | 同组主仓标记异常（非恰一行=1） | 迁移主仓（`is_main_repo`）用乐观锁（`update_at`）防并发覆盖，事务内保证组内恰一行=1；录入复制主仓配置为普通读+写，无跨行 UPDATE（见 requirement-design.md §5.3） |
