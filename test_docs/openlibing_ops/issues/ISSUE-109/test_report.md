...
> 执行证据说明：原 `execution_results/` 下 junit 结果（junit_api_batch1.xml、junit_api_batch2.xml、junit_security_batch3.xml、junit_security_019_020.xml、tc018_final.log）与失败截图（screenshots/failure_test_tc010__.png、failure_test_tc018__.png）因 2026-08-31 本地存储层异常丢失且未入库，无法恢复；逐条执行结果以本表与 test_cases.md「执行结果记录」为准（结果回填自执行时实测输出）。
