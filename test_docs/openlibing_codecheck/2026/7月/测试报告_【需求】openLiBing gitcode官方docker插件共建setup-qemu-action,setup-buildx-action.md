# openLiBing gitcode官方docker插件共建setup-qemu-action,setup-buildx-action 测试报告
...
|-------------|------|---------------------|-----|------|----------------|
| **功能测试**    | 23   | 覆盖setup-qemu-action 7个用例（默认安装、platforms、image、reset、cache-image、跨架构运行、输出验证）和setup-buildx-action 16个用例（默认创建、version、driver、driver-opts、buildkitd-flags、buildkitd-config、name、use、endpoint、platforms、cache-binary、cleanup、输出验证、append、keep-state、standalone模式）。    | 23  | 0    | Pass           |
| **集成测试**    | 4    | 两插件组合构建多架构镜像、与login-action集成、与build-push-action端到端链路、registry mirror集成。    | 4   | 0    | Pass           |
| **体验测试**    | 2    | 配置便捷性、日志可读性。    | 2   | 0    | Pass           |
| **安全与隐私测试** | 0    | -                   | 0   | 0    | Pass           |
| **可靠性与韧性测试** | 0    | -                   | 0   | 0    | Pass           |
...
- **setup-qemu-action**（GitHub v4.x）：核心功能包括安装QEMU静态二进制、注册binfmt_misc仿真器、支持platforms/image/reset/cache-image输入参数、platforms输出。已在gitcode验证用例1-7覆盖全部功能。
- **setup-buildx-action**（GitHub v4.x）：核心功能包括创建Buildx构建器、支持version/driver/driver-opts/buildkitd-flags/buildkitd-config/name/use/endpoint/platforms/append/keep-state/cache-binary/cleanup输入参数，以及name/driver/platforms/nodes输出。已在gitcode验证用例8-23覆盖全部功能。