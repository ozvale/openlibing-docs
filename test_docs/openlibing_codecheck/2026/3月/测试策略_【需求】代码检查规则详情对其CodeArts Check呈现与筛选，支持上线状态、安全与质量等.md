# #1 【需求】代码检查规则详情对其CodeArts Check呈现与筛选，支持上线状态、安全与质量等 测试策略设计说明书
...
* **核心目标**:
  完善代码检查规则筛选维度，使其包含语言、问题级别、检查引擎、是否为编译规则、安全与质量（是否付费）、业界标准、上下线状态、是非为系统规则等。
* **开发责任人**: 闫兆鸿
* **测试责任人**: 徐愚冰
...
**1.新增/编辑规则集页面筛选验证**:
* 前置条件：
* 当前版本有存量规则数据，在规则集页面操作
* 测试步骤：
* 1.进入规则集页面
* 2.点击新建规则集/点击存量规则集的编辑按钮
* 3.在规则集编辑页面筛选规则
* 预期结果：
* 规则集编辑页面筛选规则正常
