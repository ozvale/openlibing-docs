...
| [1.1-threatmodel.md](1.1-threatmodel.md)                 | Pure Mermaid DFD source file                                          |
| [1.2-threatmodel-summary.md](1.2-threatmodel-summary.md) | Summary DFD for large systems                                         |
...
用户要求对 openlibing-cicd 仓执行安全威胁分析，并将结果归档至 openlibing-docs 仓的 `architecture_desgin/openlibing-cicd` 目录。分析基线为 `release_20260831_iter2` 分支（commit `e478943ce`）。FIND-10 标记为"待验证"，需结合代码进一步确认 objectKey 归属校验。
...
| Prompt             | `Use Skill: threat-model-analyst 帮我安全威胁分析 openlibing-cicd 仓，并把结果上传到 openlibing-docs仓，归档到 architecture_desgin/openlibing-cicd下` |
