## #1 openLiBing灵枢技术架构设计说明书 (Architecture Design Document)

## 1\. 基础信息

> 本文对openLiBing灵枢技术架构设计进行说明，openLiBing是为解决蓝区开发效率问题打造的蓝区作业桌面，支撑开源社区项目看护代码质量、构件工程、自动发布。
...
![Lingshu_func.png](Lingshu_func.png)

部署架构

![Lingshu_arch.png](Lingshu_arch.png)
...
- 灵枢前端服务：openlibing-ai-web，前端服务
- 灵枢后端服务：computing-resource-workspace，包括管理面能力：项目空间管理、推理部署、API中心、一站式插件后端
- 【规划中】灵枢数据面服务：computing-resource-workspace-runtime，MaaS的数据面服务，具体具体的API路由、限流、熔断、降级等操作
- 灵枢实验室跳板机：lingshu-jumper：蓝区实验室中的代理中转服务，路由maas请求到模型实例
...
- 普通用户：可以发布模型推理服务，使用API中心
- 项目空间Owner/管理员：可以创建项目空间、管理项目空间资源和成员，可以查看项目运营页面
...
![Lingshu_STRIDE.png](Lingshu_STRIDE.png)
...
- 灵枢管理页面使用OpenLibing认证
- 灵枢MaaS模型API使用APIKey认证
- 灵枢后端服务与灵枢实验室跳板机之间建议使用双向证书认证
- 用户请求、微服务、中间件通信链路加密（TLS 1.2+）与静态加密（AES-128）全覆盖

I信息泄露：

- 对高敏感字段（API Key）进行三段式加密保存
- 屏蔽后端服务的具体报错，仅返回通用的错误代码（如 500 Internal Error）
- 请求日志Payload信息在OBS存储时进行加密，避免泄露
...
- 管数分离：管理服务和数据服务隔离，管理流量和业务流量不会相互影响（规划中）
- 熔断限流：针对每个用户设置固定的限流，依据项目空间管理员配置的并发配置模型实例限流，依据模型实例负载实现模型实例熔断
...
- 健康诊断：服务均提供/health接口
- 指标监控：灵枢后端服务记录用户调用日志，为用户提供个人用量页面。灵枢后端服务上报服务用量、负载指标到Prometheus，为业务管理员提供项目运营能力（项目运营规划中）
