...
*No Tier 1 threats identified.* Action 由 GitCode runner 调用，无 inbound 监听器；按 LOCALHOST_DESKTOP 部署分类，T1 禁止。
...
*No Tier 1 threats identified.* 进程内类，无外部入口。
...
*No Tier 1 threats identified.* 进程内类。
...
*No Tier 1 threats identified.* Python 子进程，由 SecOptionDetector spawn。
...
*No Tier 1 threats identified.* 进程内类，仅发起 outbound HTTPS。
...
*No Tier 1 threats identified.* 进程内类，不暴露接口。
...
*No Tier 1 threats identified.* 部署分类 LOCALHOST_DESKTOP 禁止 T1。
...
*No Tier 1 threats identified.* 部署分类 LOCALHOST_DESKTOP 禁止 T1；攻击者需 MITM 内网或控制 DNS 才能欺骗镜像。
...
*No Tier 1 threats identified.* LOCALHOST_DESKTOP 禁止 T1；产物写入需 PR/build 触发。
...
*No Tier 1 threats identified.* LOCALHOST_DESKTOP 禁止 T1。
